...
const Button = ({ text, primary, secondary }: ButtonProps) => {
  return (
    <div>
      {primary ? (
        <button className="bg-red-600 hover:p-3 p-3 h-10 flex cursor-pointer items-center  border-2 justify-center  ease-in duration-300  hover:bg-white text-white font-semibold hover:text-red-600 border-red-600 hover:border-2">
          {text}
        </button>
      ) : null}
      {secondary ? (
